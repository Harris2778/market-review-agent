...
**每日抓取 + 快照同步 Railway（2026-07-23 凌晨建成，同日用户决策撤销定时任务）**

决策记录：用户要求「任意股票被问都要有趋势」，全市场每日预抓物理不可行
（5000 只 × 30s ≈ 40+ 小时/天），用户拍板**不做每日定时趋势功能**；
内建半价版保留——任何股票被查询时 get_sentiment_distribution 自动落快照，
同一只股票隔几天再问即可见趋势（无需任何定时任务）。
Kimi Work 每日抓取 Automation 已删除；以下两端代码**休眠保留**（无调用方、
已测试已推送，恢复趋势功能时 30 秒可重启，也可日后清理）：

- main.py 休眠端点 POST /v1/admin/sentiment/snapshots（Bearer 鉴权，
  配额/限流口径与 /v1/admin/reports-db 一致即豁免；逐条校验+幂等 REPLACE；
  单批上限 500；绝不 500，服务端异常兜底 200+failed）
- agent/sentiment_sync.py 休眠客户端：collect_snapshots_for_date +
  push_snapshots（100 条/批，5xx 重试一次）+ sync_today 编排
  （SENTIMENT_SYNC_URL env 缺省 Railway 生产域名）
