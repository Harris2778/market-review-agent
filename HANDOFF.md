# 项目交接文档（2026-07-23 更新，第六/七波 + 新闻模式三问题修复 + 输出卫生/MCP兜底修复 + 研报库v1/v2全文RAG/每日自动化 + 开源借鉴移植四模块 + 社媒舆情爬取v1 + 东财股吧 + 清小搭实测崩坏修复 + 舆情分布化落地 + 舆情分布升级(四桶/时效窗/深度档/B站登录态)，路线图全部完成）
...
**清小搭实测崩坏修复（2026-07-22 深夜，路由+合规+[UNSOURCED]）**

事件：新功能上生产后用户实测 7 问全崩——情绪/人气/股吧/社媒问题被旧路由劫持
（「今天市场情绪」被「今天市场」触发词劫持去全复盘；「人气」被旧个股 handler 接住
只回行情卡；股吧/微博/B站落 general_chat 纯闲聊，模型按合规训练胡乱拒答；
「逆向投资」人格无人接；比较类问题流式挂起）；[UNSOURCED] 内部标记泄漏给用户。

- orchestrator.detect_intent 新增两高优先级意图（自选股判定之后、行业/复盘判定之前）：
  social_sentiment（强信号：股吧/微博/知乎/抖音/B站/哔哩/小红书/热搜/热榜/舆情/舆论
  单独命中即判；弱信号：情绪/人气/热度/关注/讨论/评论/涨停/跌停/炸板/打板/连板
  需市场语境=语境词或实体≥1）与 persona（价值投资/成长/趋势/逆向/巴菲特/格雷厄姆/
  投资框架/投资风格，需市场语境）；双保险：含「复盘」跳过 + _DATA_QUERY_COMMANDS
  指令词（查询/列出/排名等）跳过保住 mcp_query 旧路径
- 主入口：两意图走 _agent_query 并透传 hint（_agent_query 新可选参数，
  追加到系统消息尾部引导选工具：社媒四工具/analyze_with_persona 先行）；
  流式/非流式同路径
- [UNSOURCED] 修复：复盘提示词改「数据缺失写「数据未覆盖」」+ 复盘出口
  _replace_unsourced 双保险（流式带 10 字符缓冲跨 chunk 安全）
- system_prompts COMPLIANCE_PROMPT 追加社媒例外（股吧/微博/知乎/抖音/B站热点讨论
...
- 本地真实 API 复验 Q1-Q7（2026-07-22 深夜）：情绪温度 68.6 活跃+涨跌停结构、
  ...
  白酒 vs 半导体双龙头对比成稿——全部有出处、无拒答、无挂起
- 真实 E2E（2026-07-22）：茅台吧 10 帖（阅读/评论/转发/点赞真实，朱少醒调出茅台帖
  阅读 1.1 万/评论 66/点赞 28 含正文），情感 利好2/利空1/中性7

**舆情分布升级（2026-07-23 凌晨，四桶+时效窗+深度档+B站登录态）**

能力：分布从三桶升级为四桶（乐观/悲观/中性/**无关**——闲聊/广告/玩梗/纯表情
剔除出多空分母）；新增 bull_bear 多空比（仅乐观+悲观子集相对占比，无明确
多空观点时如实 None+note）；样本时效治理（since_days=7 时间窗过滤，时间缺失
条目保留宁多留不误杀；B站搜索 order=pubdate 只用于采样路径）；两档样本量
（standard≈100 帖/150 评论，deep=300 帖/400 评论/15 视频）；返回新增 window
（样本实际最早/最晚时间，禁止暗示「就是今天」）。

- sentiment_llm：VALID_LABELS 四值，无关 score 强制归 0，prompt 四标签定义
- sentiment_aggregate：BUCKETS 四桶 + _bull_bear（子集空→None+note），
  confidence 三档不变
- social_media：_parse_sample_time/_filter_by_time_window/_sample_window；
  get_sentiment_distribution(depth, since_days) 非法 depth 降级 standard 进 notes；
  tools 两工具 schema 加可选 depth 枚举；prompts 引用纪律 11-14 条
  （四桶+n 必报/bull_bear 为多空准绳/必报 window/自选择偏差声明）
- B站登录态：fetch_comments 读 BILI_SESSDATA 环境变量，有则挂 cookie+wbi 签名
  走 reply/wbi/main（mode=3 cursor 翻页）拉全量评论；签名材料失败降级带 cookie
  plain；无 SESSDATA 路径与既有完全一致，notes 透明化匿名限制；
  deep 档 video_limit 8→15 补偿匿名热评稀少
- 实测根因（2026-07-23）：B站匿名评论接口（plain/main/wbi 有无签名均试）每视频
  仅返回约 3 条热评且多为一年+旧评（被时间窗过滤），登录态才可全量——
  匿名 B站路径样本以视频标题/简介为主
- 登录态落地（2026-07-23 用户配置 BILI_SESSDATA 后实测）：
  ① wbi 评论路径 mode 3→2（按时间倒序）——热度排序 79 条中 7 天内 0 条，
  时间序 63 条中 17 条在 7 天内；② collect_keyword_samples 双路搜索精选
  （pubdate 保新鲜 + 综合排序保有评论沉淀的热门视频，「有评论优先、时间新
  优先」合并去重）——纯 pubdate 新视频大多 0 评论是 n 小的根因之一；
  ③ .env 已配 BILI_SESSDATA（gitignore 保护，约 30 天过期需重取）
- 测试：+45 用例（升级 36+登录态 8+mode=2 断言 1），全量 2029 passed 全绿
- 真实 E2E（2026-07-23）：茅台吧 standard n=80（乐观 42.5/悲观 13.8/中性 43.8/
  无关 0，多空比 75.6:24.4，窗 07-17~07-22，置信度中，19.8s）；deep n=104
  （窗过滤掉 196 条较早样本进 notes，置信度高，25.3s）；B站匿名 deep n=17
  （无关 58.8% 印证四桶价值，匿名说明落 notes，32.4s）；
  B站登录态+双路搜索 deep n=86（悲观 37.2/乐观 17.4/无关 32.6，
  多空比 31.9:68.1，窗 07-22 当天，置信度中，73.7s）

**每日抓取 + 快照同步 Railway（2026-07-23 凌晨，趋势数据积累）**

能力：本地 Mac 每日定时跑舆情分布（自选股 + 关键词），快照落本地 social.db
的同时推送到 Railway 生产实例——清小搭线上用户也能看到趋势。

- main.py 新增 POST /v1/admin/sentiment/snapshots（Bearer 鉴权，配额/限流口径
  与 /v1/admin/reports-db 一致即豁免；逐条校验+逐条 save_snapshot 幂等
  REPLACE；单批上限 500；绝不 500，服务端异常兜底 200+failed）
- agent/sentiment_sync.py：collect_snapshots_for_date（sqlite 读当日快照）+
  push_snapshots（100 条/批，5xx/网络异常重试一次，4xx 不重试）+
  sync_today 编排（base_url 走 SENTIMENT_SYNC_URL env 缺省 Railway 生产域名，
  key 走 AGENT_API_KEY，无快照/缺 key 安全返回）
- 每日任务本体在 Kimi Work Automation（不在本仓库）：本地抓取 → sync_today
  推送；终止日 2026-09-13（到期空转待续期）
- 快照表 schema 仍为三桶（pos/neu/neg 列），无关桶与 bull_bear 不落快照
  （趋势按乐观占比判定，后续如需四桶趋势再扩列）
- 测试：+31 用例（接收端 11+同步客户端 20），全量 2060 passed 全绿
- 注意：Railway 生产要用登录态需在 Variables 手动加 BILI_SESSDATA（用户操作）
...
- 股吧列表无点赞字段（仅详情页有，enrich 只回填 top_n 条）；评论数仅为计数（评论
  内容端点全灭）；详情页点赞为抓取时点快照
- 弱信号词「关注」较宽泛（「这只股票值得关注吗」也判 social_sentiment）；
  含「热搜/热榜」的裸查询由 mcp_query 改道 social_sentiment（有意改道，如需收紧
  把该词降级即可）
...
社媒可选：`BILI_SESSDATA`（B站登录 cookie，配置后评论接口走 wbi 签名拉全量；
  未配置时匿名每视频仅约 3 条热评）
