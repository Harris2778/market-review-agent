...
TOPIC_KEYWORDS = {
    "北向资金": ("北向",),
    "南向/港股通": ("南向", "港股通"),
    # 估值主题不用 PE/PB 缩写子串：QA 实锤误伤——「数据未覆盖全市场基准PE」
    # 这类只缺基准、个股估值正常的句子会误触发主题未覆盖，把正确的
    # 「茅台更便宜」头部结论替换掉。PE/PB 子串在正文出现太泛，只用全称。
    "估值": ("估值", "市盈率", "市净率"),
    "RSI": ("RSI",),
    "MACD": ("MACD",),
    "两融": ("两融", "融资余额", "融券"),
...
def _find_uncovered_topics(sentences):
    ...
    for body, _sep in sentences:
        ...
        for topic, keywords in TOPIC_KEYWORDS.items():
            if not _contains_any(body, keywords):
                continue
            # 「未覆盖全市场/行业基准」只是缺基准对照，个股估值本身可用，
            # 不算估值主题未覆盖（QA 实锤：否则头部正确结论被误替换）。
            if topic == "估值" and "基准" in body:
                continue
            topics.add(topic)
    return topics
