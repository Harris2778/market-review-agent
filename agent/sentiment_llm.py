...
标签体系为「乐观/悲观/中性/无关」四标签（词典法「利好/利空/中性」的扩展
升级），失败时整体回退 agent.sentiment 的确定性词典法，保证任何路径绝不
抛异常。

设计要点（对齐全局契约）：

1. 批量调用：每 batch_size 条拼一次 chat.completions.create，编号列表进
   prompt，要求模型只返回 JSON 数组 [{"i":编号,"label":"乐观|悲观|中性|无关",
   "score":-1~1}]。标签定义写死在 prompt：乐观=对标的/市场前景看多、利好、
   期待；悲观=看空、担忧、嘲讽看空；中性=与标的相关但无多空倾向的纯信息；
   无关=与标的投资价值无关的闲聊/广告/玩梗/纯表情。「无关」条目的 score
   一律为 0（无关不计入多空强度）。
2. 解析容忍：模型输出可带 ```json 代码块包裹/前后废话——正则提取第一个
   「[」到最后一个「]」再 json.loads；数组中单条解析失败（非 dict、编号
   非法、label 不在四值内、score 非数值）该条标中性 score=0；编号错位/缺条
...
- 悲观：看空、担忧、嘲讽看空
- 中性：与标的相关但无多空倾向的纯信息
- 无关：与标的投资价值无关的闲聊、广告、玩梗、纯表情

要求：
1. 只返回一个 JSON 数组，不要任何其他文字、解释或代码块标记；
2. 数组元素格式：{{"i": 编号, "label": "乐观|悲观|中性|无关", "score": -1到1的小数}}；
3. score 表征强度：越接近 1 越乐观，越接近 -1 越悲观，0 附近为中性；
4. label 为「无关」时 score 必须为 0（无关不计入多空强度）；
5. 每条编号都必须给出结果。
...
def _parse_entries(payload: list) -> Dict[int, dict]:
    ...
    单条容错：非 dict / 编号非法 / label 不在四值内 / score 非数值的条目
    一律丢弃（调用方按缺条补中性）；score 截断到 [-1, 1]；「无关」条目的
    score 一律归 0（无关不计入多空强度）；重复编号后者覆盖前者（等价忽略）。
    绝不抛。
    """
    ...
    for entry in payload:
        ...
        if idx is None or label not in VALID_LABELS or score is None:
            continue  # 单条解析失败：丢弃
        if label == "无关":
            score = 0.0  # 无关不计入多空强度，score 一律为 0
        parsed[idx] = {
            "label": label,
...
def score_texts_batch(texts: List[str], client=None,
    ...
    返回 [{index:int(全局下标), label:'乐观|悲观|中性|无关', score:float(-1~1),
    method:'llm'|'fallback'}]，顺序与输入一致。「无关」条目的 score 一律为 0。LLM 调用异常/超时或响应不可
